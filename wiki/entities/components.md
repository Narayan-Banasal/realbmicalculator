...
- Body Visualization: Premium multi-mesh 3D human figure (three.js WebGL) using cylinders and spheres (torso, head, arms, legs, female chest) to provide true volumetric depth, lighting, and parallax. Uses a custom ShaderMaterial to project transparent photorealistic generated full-body textures onto the front faces (via planar front-projection UV mapping in vertex shader) while rendering skin-tone or simple black clothing on side/back faces. Features automatic group viewport scaling to prevent tall height clipping, interactive yaw drag-to-rotate (preserved during inputs), and direct deformation morph updates (height, girth, limb thickness) reactive to sliders. Lives in <canvas>.
