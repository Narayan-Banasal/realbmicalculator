...
export function initBmiCalculator() {
  ...
  function applyAlert(bmi: number) {
    ...
    if (bmi >= 30) {
      title.textContent = 'Health alert — elevated BMI';
      msg.textContent =
        bmi >= 35
          ? 'Your BMI is in the severely obese range. Please consider consulting a healthcare provider.'
          : 'Your BMI is in the obese range. Lifestyle and medical guidance may help reduce long-term risk.';
    } else {
      title.textContent = 'Health notice — low BMI';
      msg.textContent =
        'Your BMI is below the healthy range. Unintended weight loss should be evaluated by a clinician.';
    }
  }
